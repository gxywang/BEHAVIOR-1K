...
Usage: ./setup_uv.sh [OPTIONS]

Options:
  -h, --help              Display this help message
  --new-env NEW_ENV_NAME  Create a uv virtual environment 'NEW_ENV_NAME' (default: .venv)
...
Example (core components): ./setup_uv.sh --new-env --omnigibson --bddl --dataset
Example (full customization): ./setup_uv.sh --new-env .venv --omnigibson --bddl --dataset --joylo --eval --primitives --cuda-version 12.6
Example (non-interactive): ./setup_uv.sh --new-env --omnigibson --dataset --accept-nvidia-eula --accept-dataset-tos
EOF
    exit 0
fi
...
# uv is the environment and package manager used by this script.
command -v uv >/dev/null 2>&1 || { echo "ERROR: uv not found. Install uv before running this script."; exit 1; }

# Use the active interpreter for every uv pip operation.
...
uv_pip() {
    ...
    if [ -n "$PYTHON_BIN" ]; then
        uv pip "$subcommand" --python "$PYTHON_BIN" "$@"
    else
        uv pip "$subcommand" "$@"
    fi
}
...
if [ "$NEW_ENV" = true ]; then
    echo "Creating uv virtual environment '$NEW_ENV_NAME'..."
    if [ -e "$NEW_ENV_NAME" ]; then
        echo ""
        echo "ERROR: uv virtual environment '$NEW_ENV_NAME' already exists!"
        echo ""
    ...
    uv venv "$NEW_ENV_NAME" --python 3.11
    source "$NEW_ENV_NAME/bin/activate"
    PYTHON_BIN="$NEW_ENV_NAME/bin/python"
    uv_pip install "setuptools>=71,<81" wheel
fi

# Install PyTorch via uv with CUDA support
...
# uv_pip install torch==2.7.0 torchvision==0.22.0 torchaudio==2.7.0 torchcodec==0.5 --index-url https://download.pytorch.org/whl/cu${CUDA_VER_SHORT}
uv_pip install torch==2.11.0 torchvision==0.26.0 torchaudio==2.11.0 --index-url https://download.pytorch.org/whl/cu${CUDA_VER_SHORT}

echo "✓ PyTorch installation completed"

# Install numpy <2 to avoid conflicts
echo "Installing numpy..."
uv_pip install "numpy<2"

# Install BDDL
if [ "$BDDL" = true ]; then
    echo "Installing BDDL..."
    [ ! -d "bddl3" ] && { echo "ERROR: bddl directory not found"; exit 1; }
    uv_pip install -e "$WORKDIR/bddl3"
fi
...
if [ "$OMNIGIBSON" = true ]; then
    ...
    uv_pip install -e "$WORKDIR/OmniGibson$EXTRAS" --no-build-isolation
    ...
    if python -c "import isaacsim" 2>/dev/null; then
        ...
        if [ "$ARCH" = "aarch64" ]; then
            uv_pip install 'isaacsim[all,extscache]==5.1.0' --extra-index-url https://pypi.nvidia.com
        else
            # Helper functions
            check_glibc_old() {
        ...
        if [ -d "$PYTHON_SITE_PACKAGES/isaacsim/extscache/omni.services.pip_archive-0.16.0+107.0.3.lx64.cp311/pip_prebundle/packaging" ]; then
            echo "Fixing packaging conflict..."
            rm -rf "$PYTHON_SITE_PACKAGES/isaacsim/extscache/omni.services.pip_archive-0.16.0+107.0.3.lx64.cp311/pip_prebundle/packaging"
        fi
    fi
    
    # Force reinstall cffi 1.17.1 to resolve compatibility issues with Isaac Sim extensions
    uv_pip install --reinstall cffi==1.17.1
    # Force reinstall websockets >= 15.0.1 because it's been overwritten by Isaac Sim with an older version
    uv_pip install --reinstall "websockets>=15.0.1"
...
if [ "$JOYLO" = true ]; then
    ...
    uv_pip install -e "$WORKDIR/joylo"
fi

# Install Eval
if [ "$EVAL" = true ]; then
    # get torch version via pip and install corresponding torch-cluster
    TORCH_VERSION=$(uv_pip show torch | grep Version | cut -d " " -f 2)
    uv_pip install torch-cluster -f https://data.pyg.org/whl/torch-${TORCH_VERSION}.html
fi
...
if [ "$ASSET_PIPELINE" = true ]; then
    ...
    uv_pip install -r "$WORKDIR/asset_pipeline/requirements.txt"
fi
...
if [ "$NEW_ENV" = true ]; then echo "✓ Created uv virtual environment '$NEW_ENV_NAME'"; fi
...
echo ""
if [ "$NEW_ENV" = true ]; then echo "To activate: source '$NEW_ENV_NAME/bin/activate'"; fi
